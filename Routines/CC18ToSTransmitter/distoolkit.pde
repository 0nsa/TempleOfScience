...
public class Broadcast {
  ...
  public void update() {
    ...
    try {
      ...
      for (int i = 0; i < nPixels; i++) {
        ...
        buffer[ofs] = byte(c & 0xFF);               // Green
        buffer[ofs + 1] = byte((c >> 16) & 0xFF);   // Red 
        buffer[ofs + 2] = byte((c >> 8) & 0xFF);    // Blue
    }
  ...
  public void draw() {
    ...
    for (int i = 0; i < nPixels; i++) {
      ...
      int g = buffer[ofs+0] & 0xFF;
      int r = buffer[ofs+1] & 0xFF;
...
public class ArtNetBroadcast extends Broadcast {
  ...
  public void update() {
    ...
    try {
      ...
      // If strips % ports > 0, our last packet won't send.
      // tracked with this boolean and sent at the end.
      ...
      for (int row = 0; row<rows; row++) {
        ...
        for (int col = 0; col<pixelMap.columns; col++) {
          ...
          // I Swaped G and B, TODO make configurable
          buffer[ofs + 2] = byte(c & 0xFF);             // Green
          buffer[ofs + 1] = byte((c >> 16) & 0xFF); // Red 
          buffer[ofs + 0] = byte((c >> 8) & 0xFF);  // Blue
        }
  ...
  public void receive(byte[] data, String ip, int port) {
    ...
    if (data[9] == 0x52) {
      ...
      // If strips % ports > 0 then our buffer size will be off
      if (size + ofs > buffer.length) {
